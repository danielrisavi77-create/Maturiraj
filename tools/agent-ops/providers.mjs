...
export async function invoke(config, { provider, task, cwd, phase, prompt, signal, schema = resultSchema }, exec = run) {
  ...
  if (provider === 'codex') {
    ...
    for (const line of r.stdout.split('\n')) { try { const e = JSON.parse(line); if (e.usage) usage = e.usage; if (e.thread_id) sessionId = e.thread_id; } catch {} }
  } else {
  ...
  return { ...result, telemetry: { provider, phase, taskId: task.id, elapsedMs: Date.now() - started, usage, sessionId, billingMode: 'subscription', extraUsageCharge: null } };
}
