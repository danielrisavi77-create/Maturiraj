...
export function isBillingCheckoutEnabled(env = process.env) {
  // Fail-closed on purpose (Tjedan 1 / P0-03). Do not set these env vars to
  // "true" on prod until Stripe + migrations are verified. UI uses
  // CHECKOUT_UI_LIVE in lib/billing/checkoutPlans.js — also keep that false.
