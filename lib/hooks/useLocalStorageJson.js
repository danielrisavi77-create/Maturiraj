...
function useStorageValue(key, defaultValue, decode, encode) {
  ...
  const setValue = useCallback(
    (update) => {
      ...
      if (currentRaw !== undefined) {
        try {
          currentValue = decode(currentRaw)
        } catch {
          currentValue = defaultValue
        }
      }
      const nextValue = typeof update === 'function' ? update(currentValue) : update
      writeRaw(key, encode(nextValue))
    },
    [key, defaultValue, decode, encode]
  )
...
export function useLocalStorageJson(key, defaultValue) {
  return useStorageValue(key, defaultValue, JSON.parse, JSON.stringify)
}

export function useLocalStorageText(key, defaultValue = '') {
  return useStorageValue(key, defaultValue, plainText, plainText)
}
