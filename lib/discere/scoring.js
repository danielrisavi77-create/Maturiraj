import { matchesNumericAnswer } from './numeric-answer.js'
import { matchesAlgebraAnswer } from './algebra-answer.js'
...
export function scoreQuestion(question, response) {
  ...
  switch (question.type) {
    case 'mc':
      return { earned: response === question.answer?.correct?.[0] ? possible : 0, possible, status: 'scored' }

    case 'multi':
      return { earned: sameSet(response, question.answer?.correct) ? possible : 0, possible, status: 'scored' }

    case 'short': {
      if (question.answer?.algebra !== undefined) return { earned: matchesAlgebraAnswer(response,question.answer) ? possible : 0, possible, status: 'scored' }
      if (question.answer?.numeric !== undefined) return { earned: matchesNumericAnswer(response,question.answer) ? possible : 0, possible, status: 'scored' }
      const accepted = (question.answer?.accepted || []).map(normalizeText)
      return { earned: accepted.includes(normalizeText(response)) ? possible : 0, possible, status: 'scored' }
    }

    case 'fill': {
      const gaps = Array.isArray(question.gaps) ? question.gaps.map((gap) => gap.id) : []
      const values = response && typeof response === 'object' ? response : {}
      const correct = gaps.map((gapId) => {
        const accepted = (question.answer?.accepted?.[gapId] || []).map(normalizeText)
        return accepted.includes(normalizeText(values[gapId]))
      })
      if (question.answer?.scoring === 'all_or_nothing') {
        return { earned: correct.length > 0 && correct.every(Boolean) ? possible : 0, possible, status: 'scored' }
      }
      const weights = splitPoints(possible, gaps, question.answer?.gapPoints)
      const earned = gaps.reduce((sum, gapId, index) => sum + (correct[index] ? weights[gapId] : 0), 0)
    ...
    case 'matching': {
      ...
      const values = response && typeof response === 'object' ? response : {}
      const correct = ids.map((id) => values[id] === question.answer?.pairs?.[id])
      if (question.answer?.scoring === 'all_or_nothing') {
        return { earned: correct.length > 0 && correct.every(Boolean) ? possible : 0, possible, status: 'scored' }
      }
      const weights = splitPoints(possible, ids, question.answer?.pairPoints)
      const earned = ids.reduce((sum, id, index) => sum + (correct[index] ? weights[id] : 0), 0)
      return { earned, possible, status: 'scored' }
    }

    case 'ordering':
      return { earned: exactOrder(response, question.answer?.order) ? possible : 0, possible, status: 'scored' }
...
export function scoreExam(exam, responses = {}) {
  ...
  let autoPossiblePoints = 0
  let manualPossiblePoints = 0
  ...
  for (const question of leaves) {
    ...
    if (scored.status === 'manual') {
      manualPending = true
      manualPossiblePoints += scored.possible
    } else if (scored.status === 'scored') {
      autoPossiblePoints += scored.possible
    }
  ...
  const autoPercent = autoPossiblePoints > 0 ? Math.round((earnedPoints / autoPossiblePoints) * 10000) / 100 : 0
...
export function buildResult({ exam, responses = {}, startedAt, completedAt, flagged = [] }) {
  ...
  return {
    ...
    autoPossiblePoints: scored.autoPossiblePoints,
    manualPossiblePoints: scored.manualPossiblePoints,
    autoPercent: scored.autoPercent,
    answers: responses,
