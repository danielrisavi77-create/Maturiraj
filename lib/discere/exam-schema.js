...
function validateMeta(meta, errors) {
  ...
  if (!Number.isInteger(meta.year) || meta.year < 2000) errors.push(issue('INVALID_YEAR', 'meta.year', 'Godina ispita nije valjana.'))
  if (!isNonEmptyString(meta.season)) errors.push(issue('SEASON_REQUIRED', 'meta.season', 'Rok/sezona je obvezna.'))
  ...
  if (!meta.source || !isNonEmptyString(meta.source.provider) || !isNonEmptyString(meta.source.examUrl) || !isNonEmptyString(meta.source.retrievedAt)) {
...
export function validateExam(exam, options = {}) {
  ...
  exam.questions.forEach((question, index) => validateQuestion(question, `questions[${index}]`, state, options))
