...
import { isNumericAnswer, parseRational } from './numeric-answer.js'
...
function validateAssets(question, path, errors) {
  ...
  question.assets.forEach((asset, index) => {
    ...
    if (asset.type === 'audio' && asset.source === undefined) {
...
// relativeTolerance se provjerava ovdje jer numeric-answer.js prihvaća samo svoj skup ključeva.
function validateNumeric(answer, path, errors) {
  ...
  if (isPlainObject(rule) && rule.relativeTolerance !== undefined) {
    const parsed = parseRational(rule.relativeTolerance)
    if (!parsed || parsed.n < 0n) {
      errors.push(issue('INVALID_RELATIVE_TOLERANCE', `${path}.answer.numeric.relativeTolerance`, 'relativeTolerance mora biti nenegativan decimalni ili razlomački string.'))
      return
    }
    const rest = { ...rule }
    delete rest.relativeTolerance
    probe = { ...answer, numeric: rest }
  }
  if (!isNumericAnswer(probe)) {
