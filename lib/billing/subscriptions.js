...
import { getEffectiveTier, TIER_RANK } from '@/lib/billing/getEffectiveTier'

// ─── Constants ───────────────────────────────────────────────────────────────

export const FREE_PREVIEW_LIMIT = 3

// TIER_RANK i pravilo isteka žive u getEffectiveTier.js (jedan izvor istine);
// ovdje se samo re-exportaju zbog postojećih importa.
export { TIER_RANK } from '@/lib/billing/getEffectiveTier'

const PLAN_SLUG_TO_TIER = Object.freeze({
  starter: 'starter',
  pro:     'pro',
  pro_god: 'pro',
})
...
function buildPriceMap() {
  ...
  const register = (priceId, tier, source) => {
    if (!priceId) return

    const existingTier = priceMap.get(priceId)
    if (existingTier && existingTier !== tier) {
      throw new Error(`[billing] Stripe price ${priceId} has conflicting tier mappings (${existingTier}/${tier}) via ${source}`)
    }

    priceMap.set(priceId, tier)
  }

  // Canonical checkout env names.
  register(process.env.NEXT_PUBLIC_STRIPE_STARTER_MONTHLY_PRICE_ID, 'starter', 'NEXT_PUBLIC_STRIPE_STARTER_MONTHLY_PRICE_ID')
  register(process.env.NEXT_PUBLIC_STRIPE_PRO_MONTHLY_PRICE_ID, 'pro', 'NEXT_PUBLIC_STRIPE_PRO_MONTHLY_PRICE_ID')
  register(process.env.NEXT_PUBLIC_STRIPE_PRO_YEARLY_PRICE_ID, 'pro', 'NEXT_PUBLIC_STRIPE_PRO_YEARLY_PRICE_ID')

  // Backwards-compatible server aliases. Conflicting tiers fail closed.
  register(process.env.STRIPE_PRICE_STARTER, 'starter', 'STRIPE_PRICE_STARTER')
  register(process.env.STRIPE_PRICE_PRO, 'pro', 'STRIPE_PRICE_PRO')
  register(process.env.STRIPE_PRICE_PRO_GOD, 'pro', 'STRIPE_PRICE_PRO_GOD')
...
export function derivePlanFromStripe(stripeSub) {
  // A paid tier is granted only when the Stripe price itself is allowlisted.
  // Metadata is supplementary and may never override an unknown price.
  const meta = stripeSub.metadata?.plan
  const priceId = stripeSub.items?.data?.[0]?.price?.id
  const priceMap = buildPriceMap()

  if (!priceId || !priceMap.has(priceId)) {
    throw new Error(`[billing] Unrecognised Stripe price (plan=${meta ?? 'none'}, price=${priceId ?? 'none'})`)
  }

  const priceTier = priceMap.get(priceId)
  if (meta) {
    const metadataTier = PLAN_SLUG_TO_TIER[meta]
    if (!metadataTier) {
      throw new Error(`[billing] Unrecognised Stripe metadata plan ${meta}`)
    }
    if (metadataTier !== priceTier) {
      throw new Error(`[billing] Stripe metadata/price tier mismatch (${metadataTier}/${priceTier})`)
    }
  }
...
export async function deactivateSubscription(stripeSubscriptionId, supabase) {
  ...
  if (sub?.user_id) {
    const { error: profErr } = await db
      ...
      .eq('id', sub.user_id)
    // Bez provjere: otkazani korisnik je zadržavao Pro ako profiles update tiho padne.
    if (profErr) throw new Error(`[billing] deactivateSubscription profiles: ${profErr.message}`)
  }
}
...
export async function getUserTier(userId) {
  ...
  // Isto pravilo (plan + istek) kao u proxyju i useAuth-u.
