...
export function score(qs, secrets, answers) {
  ...
  // OSNOVICA OCJENE = automatski ocijenjeni dio, kao kod engleskog.
  //
  // `summary.percent` dijeli s `maxPoints`, a u njemu su i bodovi ručnih zadataka
  // koji u trenutku predaje nose earned=0 (esej je `status: 'manual'`). Učenik
  // koji točno riješi sve automatski ocjenjivo i napiše esej od 33 % bodova time
  // dobiva 67 % i ocjenu 2, dok bi eng adapter za istu izvedbu dao 100 % i 5 —
  // MANUAL_TYPES tamo ne ulaze u nazivnik i komentar to zove ugovorom. Ljestvica
  // (85/70/55/40) jest zajednička, pa mora biti i osnovica; inače canonical
  // predmet trajno upisuje deflatiran `score_pct` u sim_progress.
  //
  // `scoreExam` te brojeve već daje (`autoPercent`, `manualPossiblePoints`).
  // Bez ručnih zadataka ostaje `percent`, jer on poštuje `meta.maxPoints`.
  const basisPercent = summary.manualPossiblePoints > 0 ? summary.autoPercent : summary.percent
  const pct = Math.max(0, Math.min(100, Math.round(basisPercent)))
  ...
  return {
    ...
    progressRow: {
      result_version: 2,
      // score_pct je osnovica ocjene (automatski dio); max_points je PUNI zbroj
      // s ručnima — po njima se poslije ručnog pregleda rezultat može dopuniti.
      score_pct: pct,
