...
export function mergeSecrets(question, secrets) {
  ...
  const out = hasSecret ? { ...question, ...secret } : { ...question }
...
function mergeNodeWithBudget(question, secrets, state) {
  ...
  const children = question.children
  if (Array.isArray(children) && children.length) {
    const mergedBefore = state.merged
    let changed = false
    ...
    // Grupa svoju tajnu (npr. transkript slušanja) dobiva samo ako je ijedan
    // njezin list doista dobio ključ — inače bi free preview otvorio snimku
    // cijele grupe zbog jednog lista iza granice.
    const groupSecret = state.merged > mergedBefore && secrets ? secrets[question.id] : null
    const hasGroupSecret = !!groupSecret && typeof groupSecret === 'object'
    if (!changed && !hasGroupSecret) return question
    const out = hasGroupSecret ? { ...question, ...groupSecret } : { ...question }
