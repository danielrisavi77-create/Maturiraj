...
import { createCanonicalAdapter } from './subjects/canonical.js'
...
/**
 * Predmeti koji još nemaju adapter, ali rute za njih moraju biti definirane.
 *
 * `loadPublic` vraća null, pa ruta odgovara 404 — isto što bi napravila i za
 * neregistriran predmet. Razlika je u tome što je predmet od sada NAVEDEN na
 * jednom mjestu: migracija legacy enginea (val C: mat, hrv) mijenja samo ovaj
 * redak, bez ijednog novog uvoza u registar.
 *
 * @param {string} subject
 */
function createStubAdapter(subject) {
  return Object.freeze({
    ...
    score: () => {
      throw new Error(`exam-secrets: predmet ${subject} još nema adapter za ocjenjivanje.`)
    },
  })
}

/** Legacy predmeti koji čekaju svoj adapter (val C migracije ADR-001). */
const STUB_SUBJECTS = Object.freeze(['mat', 'hrv'])
 ...
 *
 * Canonical predmeti se NE nabrajaju ručno: izvor istine je
 * lib/discere/subject-registry.js (`runtime: 'canonical'`), pa novi predmet
 * dobiva rutu čim uđe u registar. Ispit koji još nije generiran svejedno daje
 * 404 — `loadPublic` ne nađe content/<id>/exams/<key>.json.
 */
export function registerBuiltInSubjects() {
  registerSubject('eng', engAdapter)
  registerSubject('soc', socAdapter)
  for (const subject of DISCERE_SUBJECTS) {
    if (subject.runtime === 'canonical') registerSubject(subject.id, createCanonicalAdapter(subject.id))
  }
  for (const subject of STUB_SUBJECTS) {
    if (!adapters.has(subject)) registerSubject(subject, createStubAdapter(subject))
  }
}
