...
/**
 * Broj ispitnih cjelina prema LAGANOM indeksu (bez učitanih pitanja). Koristi se
 * samo za natpis na kartici „Nastavi ispit” — pravu odluku (nastavak sljedeće
 * cjeline ili predaja) donosi getExamBlocks nad učitanim ispitom.
 *
 * Pravilo je isto kao u getExamBlocks: blok postoji ako ispit ima pitanja te
 * vrste. Indeks nosi samo 'hasReading' i 'hasListening', a pisanje u bankama
 * postoji uvijek kad postoji i čitanje (provjereno nad exams-*.json), pa blok
 * Pisanja prati zastavicu čitanja.
 *
 * @param {{razina?: string, hasReading?: boolean, hasListening?: boolean}} rec
 * @returns {number}
 */
export function blockCountFromIndex(rec) {
  if (!rec) return 0
  return structureFor(rec.razina).blocks
    .filter(b => (b.sections.includes('listening') ? rec.hasListening !== false : rec.hasReading !== false))
    .length
}
