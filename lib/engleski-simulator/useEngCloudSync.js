...
import {
  ENG_USER_KEY, ENG_BOOKMARKS_KEY, ENG_BOOKMARKS_DELETED_KEY, ENG_SYNCED_AT_KEY, ENG_CLOUD_UID_KEY,
...
export function useEngCloudSync({ user, userData, setUserData }) {
  ...
  useEffect(() => {
    ...
    const hydrate = async () => {
      ...
      const { userData: cloudUser, bookmarks: cloudBm, bookmarksDeleted: cloudDel, savedAt: cloudAt } = parseCloudBlob(blob)
      let storedUid = null
      ...
      if (ownership === 'foreign') {
        ...
        try { localStorage.removeItem(ENG_BOOKMARKS_DELETED_KEY) } catch {}
        ...
        if (cloudUser) {
          ...
          try { localStorage.setItem(ENG_BOOKMARKS_DELETED_KEY, JSON.stringify(cloudDel)) } catch {}
        ...
        let localDel = {}
        try { localDel = JSON.parse(localStorage.getItem(ENG_BOOKMARKS_DELETED_KEY) || '{}') } catch {}
        const { bookmarks: mergedBm, deleted: mergedDel } = mergeBookmarks(localBm, localDel, cloudBm, cloudDel)
        try { localStorage.setItem(ENG_BOOKMARKS_KEY, JSON.stringify(mergedBm)) } catch {}
        try { localStorage.setItem(ENG_BOOKMARKS_DELETED_KEY, JSON.stringify(mergedDel)) } catch {}
        if (cloudAt > 0) { try { localStorage.setItem(ENG_SYNCED_AT_KEY, String(cloudAt)) } catch {} }
        ...
        if (localUser && Array.isArray(localUser.history) && localUser.history.length) {
          ...
          try { localDel = JSON.parse(localStorage.getItem(ENG_BOOKMARKS_DELETED_KEY) || '{}') } catch {}
          const at = Date.now()
          const saved = await saveEngCloudState(buildCloudBlob({ userData: localUser, bookmarks: localBm, bookmarksDeleted: localDel, savedAt: at }))
          if (cancelled) return
  ...
  useEffect(() => {
    ...
    const doSave = async () => {
      ...
      let del = {}
      try { del = JSON.parse(localStorage.getItem(ENG_BOOKMARKS_DELETED_KEY) || '{}') } catch {}
      const at = Date.now()
      const saved = await saveEngCloudState(buildCloudBlob({ userData: userData || {}, bookmarks: bm, bookmarksDeleted: del, savedAt: at }))
