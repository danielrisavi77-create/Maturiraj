 ...
 *
 * Pitanje bez `sol` daje null, ne iznimku: otkako ispit dolazi s poslužitelja
 * (ADR-001), free korisnik dobiva pitanja bez ključa, pa bi `q.sol.cl` ovdje
 * značio bijeli ekran umjesto tihe degradacije. Pozivatelji svugdje uspoređuju
 * s `=== true`, pa null i dalje znači "nije točno".
 */
...
/**
 * Zamjena za `chk` kad je ispit ocijenio poslužitelj: umjesto ključa gleda mapu
 * `scores` (qid → true|false|null) iz odgovora `POST /api/sim/eng/grade`.
 *
 * Time isti kod (sectionScores, topic_breakdown, errorTracker, bojenje mrežice)
 * radi i bez ijednog ključa u pregledniku, a ADR točka 10 vrijedi doslovno:
 * klijent ne računa ocjenu koju je poslužitelj već dao, nego je samo raspoređuje.
 *
 * @param {Record<string, boolean|null>|null|undefined} scores
 * @returns {(q: object) => true|false|null}
 */
export function scoreLookup(scores) {
  const map = scores && typeof scores === "object" ? scores : {};
  return function chkFromScores(q) {
    const value = q ? map[q.id] : undefined;
