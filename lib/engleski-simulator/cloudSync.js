...
export function toSimProgressPayload(result, topic_breakdown, examRazina) {
  ...
  return {
    ...
    errorTags: [],
    // Otisak pokušaja (ADR-001). Rezervni upis se radi SAMO kad ocjenjivačka ruta
    // nije odgovorila — a ona je pokušaj možda ipak ocijenila i upisala. Bez
    // attempt_id djelomični jedinstveni indeks (user_id, attempt_id) nema na čemu
    // raditi (vrijedi tek `where attempt_id is not null`), pa bi NULL redak prošao
    // i isti pokušaj bi se brojao dvaput (povijest, totalExams, percentil).
    attemptId: typeof result.attemptId === 'string' && result.attemptId ? result.attemptId : null,
  }
}
