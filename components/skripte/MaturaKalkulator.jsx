...
function calcEvaluate(expr, angle) {
  let e = expr
    ...
    .replace(/\^/g, '**')
    // Lookbehind (?<![a-zA-Z]) sprječava da sin/cos/tan pogode podniz unutar asin/acos/atan.
    .replace(/(?<![a-zA-Z])sin\(/g, angle === 'DEG' ? 'Math.sin((Math.PI/180)*(' : 'Math.sin((')
    .replace(/(?<![a-zA-Z])cos\(/g, angle === 'DEG' ? 'Math.cos((Math.PI/180)*(' : 'Math.cos((')
    .replace(/(?<![a-zA-Z])tan\(/g, angle === 'DEG' ? 'Math.tan((Math.PI/180)*(' : 'Math.tan((')
    ...
    // log10 PRIJE ln — inače `ln(`→`Math.log(` pa ga `log(` zamjena pretvori u `Math.Math.log10(`.
