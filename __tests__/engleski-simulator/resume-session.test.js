...
// Isti ispit kakvim ga po ADR-001 vidi klijent free/standard tiera: BEZ ijednog
// 'sol'. Na njemu lokalni izračun ne postoji, pa se vidi što se dogodi kad
// poslužiteljsko ocjenjivanje padne.
const NOKEY_EXAM = {
  ...OSN_EXAM,
  key: '2023_ljeto',
  year: 2023,
  qs: OSN_EXAM.qs.map(({ sol, ...rest }) => rest),
}
const EXAMS_MAP = { [OSN_EXAM.key]: OSN_EXAM, [VIS_EXAM.key]: VIS_EXAM, [NOKEY_EXAM.key]: NOKEY_EXAM }
...
const EXAMS_INDEX = [
  ...
  { key: NOKEY_EXAM.key, year: 2023, season: 'ljeto', label: 'Ljetni rok', razina: 'osnovna', hasListening: false, hasReading: true, qCount: 2 },
  { key: VIS_EXAM.key, year: 9999, season: 'test', label: 'Testni rok', razina: 'visa', hasListening: true, hasReading: true, qCount: 6 },
]

// Nastavak sesije dovlači SAMO svoj ispit (ensureExam → loadExamByKey, ADR-001
// Faza 1), pa mock mora imati i te dvije funkcije — ne samo dohvat cijele razine.
vi.mock('@/lib/engleski-simulator/examsLoader', () => ({
  RAZINE: ['osnovna', 'visa'],
  getExamsIndex: () => EXAMS_INDEX,
  isExamLoaded: () => true,
  loadExamByKey: () => Promise.resolve(EXAMS_MAP),
  isRazinaLoaded: () => true,
...
describe('ExamPlayScreen — snapshot i nastavak', () => {
  beforeEach(() => {
    ...
    // Predaju ocjenjuje poslužitelj (ADR-001 Faza 1), pa i ovdje ide preko
    // dvojnika rute `POST /api/sim/eng/grade` — klijent rezultat ne računa sam.
    installSimApiMock({ exams: { [VIS_EXAM.key]: makeVisaExam(), [OSN_EXAM.key]: OSN_EXAM }, tier: 'pro' })
  ...
  it('predaja ispita briše snapshot (nakon poslužiteljskog ocjenjivanja)', async () => {
    vi.useFakeTimers()
    const { onDone } = renderPlay({ examMode: false })
    expect(snap()).not.toBeNull()
    fireEvent.click(screen.getByRole('button', { name: 'Vidi rezultate' }))
    // Snapshot pada tek kad rezultat stvarno stigne: pala predaja ga mora
    // ostaviti, inače bi pokušaj nestao zajedno s mrežom.
    await act(async () => {})
    expect(onDone).toHaveBeenCalledTimes(1)
    expect(onDone.mock.calls[0][0].serverSaved).toBe(true)
    expect(snap()).toBeNull()
  ...
  it('pala predaja ČUVA snapshot — pokušaj se nastavlja s Homea', async () => {
    vi.useFakeTimers()
    // 500 nije „pričekaj” nego pad: bez punih ključeva rezultat može dati samo
    // poslužitelj, pa odgovori moraju ostati zapisani.
    installSimApiMock({ exams: { [VIS_EXAM.key]: makeVisaExam() }, tier: 'free', gradeStatus: 500 })
    const noKeys = { ...makeVisaExam(), qs: makeVisaExam().qs.map(({ sol, ...rest }) => rest) }
    const { onDone } = renderPlay({ exam: noKeys, examMode: false })
    fireEvent.click(screen.getByRole('button', { name: 'Vidi rezultate' }))
    await act(async () => {})
    expect(onDone).not.toHaveBeenCalled()
...
describe('EngleskiSimulator — Home kartica „Nastavi ispit”', () => {
  // Automatska predaja istekle simulacije ide istom rutom kao i predaja iz
  // ispita (`POST /api/sim/eng/grade`), pa Home testovi dižu isti dvojnik rute;
  // on ujedno servira i exam-context.json.
  let api
  beforeEach(() => {
    api = installSimApiMock({ exams: EXAMS_MAP, tier: 'pro' })
    try { localStorage.clear() } catch {}
    window.confirm = vi.fn(() => true)
  })
  afterEach(() => {
  ...
  it('istekla ZADNJA cjelina → automatska predaja spremljenih odgovora i poruka', async () => {
    ...
    expect(snap()).toBeNull()
    // Ocijenio je POSLUŽITELJ, istom rutom i istim oblikom tijela kao predaja iz
    // ispita — klijent nema ključeve pa rezultat ni ne može izračunati sam.
    expect(api.calls.grade).toHaveLength(1)
    expect(api.calls.grade[0]).toMatchObject({
      examKey: OSN_EXAM.key, answers: { o1: 'A' }, examMode: true,
    })
    expect(typeof api.calls.grade[0].attemptId).toBe('string')
  ...
  it('pala automatska predaja ne gubi pokušaj — kartica i dalje nudi predaju', async () => {
    // Ocjenjivanje pada (500), a klijent nema ključeve: jedini pošten ishod je
    // poruka i SAČUVAN snapshot, nikako rezultat izmišljen na klijentu.
    api.setGradeOutcome(500)
    writeActiveSession(buildSnapshot({
      examKey: NOKEY_EXAM.key, examMode: true, answers: { o1: 'A' }, cur: 0,
      endsAt: Date.now() - 5 * 60 * 1000,
    }))
    render(e(EngleskiSimulator))
    await screen.findByText('Ispit čeka predaju', {}, { timeout: 20000 })
    ...
    await waitFor(() => expect(api.calls.grade.length).toBe(1), { timeout: 20000 })
    await waitFor(
      () => expect(document.querySelector('[role="status"]')?.textContent || '').toContain('Slanje rezultata nije uspjelo'),
      { timeout: 20000 },
    )
    expect(document.querySelector('.score-ring-pct')).toBeNull()
    expect(snap()).not.toBeNull()
    expect(screen.getByText('Ispit čeka predaju')).toBeTruthy()
  }, 90000)
