...
export async function validateSubjectDirectory(subjectDir) {
  ...
  for (let i = 0; i < paths.length; i += 1) {
    ...
    exams.push({ key: entry.key, valid: result.valid, errorCount: result.errors.length, warningCount: result.warnings.length })
...
export async function runDiscereValidation({ cwd = process.cwd(), subject = null } = {}) {
  ...
  if (subject && dirs.length === 1 && !existsSync(join(dirs[0], 'index.json'))) {
    ...
    printReport(report)
    return { valid: true, reports: [report] }
  }
  ...
  for (const dir of dirs) {
    const report = await validateSubjectDirectory(dir)
    reports.push(report)
    printReport(report)
  }
