...
import { getSubjectCategory } from '@/lib/data/subjectCategories'

// Generički prikaz za bilo koji predmet čiji je sadržaj u strukturiranoj shemi
// (module -> chapters -> {mustKnow, quickConcept, theory, taskPatterns, formulas,
// tasks, quiz, ...}). Izvučeno iz KemijaViewer-a, koji je bio ~99% generičan —
// vidi ../kem/KemijaViewer.jsx za tanki wrapper koji čuva kemijino ponašanje.

const CATEGORY_LABEL = {
  jezici: 'Jezik',
  prirodni: 'Prirodoslovno',
  drustveni: 'Društveno-humanističko',
  umjetnost: 'Umjetnost',
}
...
// ─── ChapterView ──────────────────────────────────────────────────────────────
function ChapterView({ subject, chapter, module_, allChapters, onBack }) {
    ...
    <div style={{ maxWidth: 760, margin: '0 auto', padding: '28px 20px 80px' }}>
      ...
      <div style={{ marginBottom: 32 }}>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 14 }}>
          <span style={badge(subject?.color || '#34d399')}>{subject?.name}</span>
...
// ─── StructuredSkriptaViewer (main export) ─────────────────────────────────────
// kicker/tagline su opcionalni — bez njih se izvode iz predmeta/kategorije, pa
// ovu komponentu novi predmeti mogu koristiti izravno bez posebnog wrappera.
export default function StructuredSkriptaViewer({ subject, onBack, kicker, tagline }) {
  const [selectedModule, setSelectedModule] = useState(null)
  const [selectedChapter, setSelectedChapter] = useState(null)

  const resolvedKicker = kicker || CATEGORY_LABEL[getSubjectCategory(subject?.id)] || 'Skripta'
  const resolvedTagline = tagline || `Kompletna priprema za državnu maturu iz predmeta ${subject?.name || ''}.`
  const accentColor = subject?.color || '#34d399'

  // Fallback: subject nema `modules`, samo `razine[].poglavlja[]`.
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', color: 'var(--text)', fontFamily: 'var(--font-sans, sans-serif)' }}>
      <div style={{ maxWidth: 1080, margin: '0 auto', padding: '40px 24px 80px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 16 }}>
          <div style={{ width: 56, height: 56, borderRadius: 16, background: `${accentColor}1a`, border: `1px solid ${accentColor}40`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 26 }}>
            {subject?.sym || '📘'}
          </div>
          <div>
            <div style={{ fontSize: 11, fontWeight: 800, letterSpacing: '.1em', textTransform: 'uppercase', color: accentColor, marginBottom: 4 }}>{resolvedKicker}</div>
            <h1 style={{ fontSize: 'clamp(28px,4vw,42px)', fontWeight: 800, letterSpacing: '-.03em', lineHeight: 1.05, margin: 0 }}>{subject?.name}</h1>
          </div>
        </div>
