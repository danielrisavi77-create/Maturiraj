...
export const metadata = {
  code: 'H19',
  title: 'H19 · Fonetika i fonologija — Maturiraj.hr',
  description: 'H19 Fonetika i fonologija — glasovi, glasovne promjene, 4 naglaska, IPA, scanner fonema. Za hrvatsku maturu 2026.',
  canonical: 'https://maturiraj.hr/hrvatski/h19-fonetika',
  ogImage: 'https://maturiraj.hr/og/h19-fonetika.png',
};
...
<nav class="sidebar" id="sidebar" role="navigation" aria-label="Navigacija poglavlja">
  ...
  <a class="sb-item active" aria-current="page" data-code="h19" href="/skripte/hrv/h19"><span class="sb-dot"></span>H19 · Fonetika i fonologija</a>
...
function scanWord(){
  ...
  out.innerHTML = \`
    ...
    <div style="padding:16px;background:var(--ele);border:1px solid var(--bd);border-radius:var(--r1);margin:12px 0;text-align:center">
      ...
      <div style="font-family:'Courier New',monospace;font-size:22px;color:var(--t1)">/\${ipa}/</div>
    </div>
        ...
        <tbody>
          \${tokens.map((t, i) => {
            ...
            return \`<tr style="border-bottom:1px solid var(--bd);\${isVowel ? 'background:rgba(233,180,70,.05)' : ''}">
              ...
              <td style="padding:8px;text-align:center;font-family:'Courier New',monospace;font-size:14px;color:var(--blue)">[\${p.ipa}]</td>
...
/* === H19 PREMIUM VIZUALNE NADOGRADNJE === */

/* Hero — više dramatičan gradient */
...
.diag-opt.correct{
  ...
  background:rgba(107,148,100,.12);
  color:var(--green);
  font-weight:600;
}
.diag-opt.wrong{
  border-color:var(--red);
  background:rgba(192,66,68,.12);
  color:var(--red);
}
...
/* Hero polish — make sure chips look consistent */
.hero-chips{
  display:flex;
  flex-wrap:wrap;
  gap:8px;
  ...
  useEffect(() => {
    ...
    if (!scriptsExecutedRef.current && SCRIPTS_JS) {
      try {
        ...
        console.error(`[H19] Script execution error:`, err);
      }
    }

    return () => {
      // Cleanup on unmount
      const styleEl = document.getElementById(`chapter-styles-H19`);
